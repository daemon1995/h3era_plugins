...
{
    const auto warehouse = extender::GetFromMapItem<H3MapItemWarehouse>(mapItem);
...
BOOL WarehousesExtender::InitNewGameMapItemSetup(H3MapItem *mapItem, int typeCount, int subtypeCount) const noexcept
{
    const auto warehouse = extender::GetFromMapItem<H3MapItemWarehouse>(mapItem);
    *warehouse = {};
...
{
    auto objSetup = extender::GetFromMapItem<H3MapItemWarehouse>(mapItem);
    ...
    const INT16 resNum = objSetup->resourceAmount
                             ? resType == eResource::GOLD ? objSetup->resourceAmount * 500 : objSetup->resourceAmount
                             : -1;
