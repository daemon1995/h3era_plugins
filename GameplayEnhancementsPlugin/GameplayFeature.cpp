...
namespace features
{
    ...
    H3DlgDefButton* __stdcall H3DlgDefButton__Ctor(HiHook* h,
        H3DlgDefButton* bttn,
        int PosX,
        int PosY,
        int SizeX,
        ...
        int Flags)
    {
        ...
        H3DlgDefButton* result = THISCALL_12(H3DlgDefButton*, h->GetDefaultFunc(), bttn, PosX, PosY, SizeX, SizeY, ItemInd, DefName, cadre, pressCadre, CloseDialog, HotKey, Flags);


        // check base conditions as std "OK_BTTN_ID" and not H3TownDlg's defName ("tsbtns.def")
        if (result
            && (ItemInd == eControlId::OK || ItemInd == 30722 || HotKey == eVKey::H3VK_ENTER)
            && DefName != reinterpret_cast<char*>(IntAt(0x05C5A08 + 1))
            )
        {
    ...
    signed int __stdcall H3HeroDlg_Main(HiHook* h, const int heroId, int hideDelButton, int isKingdomOverView, const int isRightClick)
    {
        ...
        if (hideDelButton
            && !isRightClick
            && P_ActivePlayer->ownerID == P_CurrentPlayerID
