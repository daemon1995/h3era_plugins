...
namespace scroll
{
	...
	int __stdcall MapScroller::AdvMgr_MapScreenProcedure(HiHook* h, H3AdventureManager* adv, H3Msg* msg) noexcept
	{
		...
		if (mapScroller.isMapView
			...
			(
				...
				&& msg->subtype == eMsgSubtype::MOUSE_WHEEL_BUTTON_DOWN
				)
	...
	int __stdcall MapScroller::AdvMgr_MouseMove(HiHook* h, H3AdventureManager* adv, int x, int y) noexcept
	{
		...
		auto& mapScroller = Get();
		if (
			mapScroller.rmcAtMapScreen && STDCALL_1(SHORT, PtrAt(0x63A294), VK_RBUTTON) & 0x800
			|| mapScroller.wheelButtonAtMapScreen && STDCALL_1(SHORT, PtrAt(0x63A294), VK_MBUTTON) & 0x800
			)  // if rmcIs held
		{
			if (!adv->centeredHero)
			{
