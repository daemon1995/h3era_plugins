...
std::vector<std::string> gameModList;
...
_LHF_(SelectScenarioDlg_Campaign_BeforeRun)
{
    ...
    if (dlg)
    {
        H3DlgCaptionButton *bttn = dlg->CreateCaptionButton(600, 199 + 45, 24, 14, 4500, "Iokay.def", "ERA options",
                                                            h3::NH3Dlg::Text::SMALL, 0);
        if (bttn)
        {
            bttn->SetClickFrame(1);
            bttn->AddHotkey(h3::eVKey::H3VK_E);
...
_LHF_(HooksInit)
{
    ...
    gameModList = modList::GetEraModList(false);
