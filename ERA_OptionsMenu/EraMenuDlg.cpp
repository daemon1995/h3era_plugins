...
BOOL CreateAndRunEraMenuDlg()
{
    ...
    return EraMenuDlg::CreateAndRun();
}

EraMenuDlg::EraMenuDlg(
    int width, int height,
    std::vector<std::string> &modNames) // int x, int y, BOOL statusBar, BOOL makeBackGround, INT32 colorIndex)
    : H3Dlg(width, height, -1, -1, 1, 1), MenuItem("dlg") //, -1, -1, false, makeBackGround, colorIndex)
{
...
bool EraMenuDlg::NeedRestart() const
{
...
BOOL EraMenuDlg::DialogProc(H3Msg &msg)
{
    ...
    if (this->focusedItemId == m_searchWidget->GetID() && msg.KeyPressed())
    {
        //	m_searchWidget->Draw();
        // m_searchWidget->Refresh();
        m_searchWidget->ParentRedraw();
        // Redraw();
        GetHintBar()->ShowMessage(h3_NullString);
    }
    if (msg.IsLeftDown() && msg.ItemAtPosition(this) == m_searchWidget
        //::ITEM_COMMAND
...
void __fastcall Era_Dlg_Scroll_Proc(INT32 a1, H3BaseDlg *dlg)
{
    dlg->GetScrollbar(SCROLL_BAR_ID)->Refresh();
} // return;
...
BOOL EraMenuDlg::CreateWidgets()
{
    ...
    {
        H3DlgDefButton *bttn =
            H3DlgDefButton::Create(i * 60 + 30, 40, 110 + i, "icm003.def", 0, 1, 0, eVKey::H3VK_1 + i);

        AddItem(bttn);
        for (size_t j = 0; j < 20; j++)
        {
            //	H3DlgFrame* fr = H3DlgFrame::Create(32 *j +24, i * 60 + 24, 24, 24, j* i +j + 1, h3::H3RGB565::Black() +
            // i * 12 + (j * 5 - i)); 	AddItem(fr);
        }
    }
    constexpr int step = 1;
    H3DlgScrollbar *scroll = H3DlgScrollbar::Create(INT32(widthDlg * 0.25f), 14, 16, heightDlg - 46, SCROLL_BAR_ID, 6,
                                                    Era_Dlg_Scroll_Proc, false, step, true);
    AddItem(scroll);
    // h3dlgc

    //	H3Messagebox(Era::tr(dlg_mods[dlg_mods.size()-1].name.String()));
    m_searchWidget =
        CreateEdit(600, 25, 150, 20, 20, "{~808080}Search}", h3::NH3Dlg::Text::MEDIUM, 1, 5, "ovtitle.pcx", 777);
    m_searchWidget->SetCaret(11);
    m_searchWidget->SetAutoredraw(true);
    // AddItem(scroll);
...
BOOL EraMenuDlg::CreateAndRun()
{
    ...
    EraMenuDlg *ptr = nullptr;
    do
    {

        {
            EraMenuDlg dlg(width, height, gameModList);
            ptr = &dlg;
            dlg.Start();
...
{
    EraMenuDlg::CreateAndRun();
}

// PatcherInstance* _PI = nullptr;
BOOL EraMenuDlg::CreateMods(EMod &mod, int i)
{
    H3DlgText *txt = nullptr;
    for (auto &m : mods)
    {
        txt = H3DlgText::Create(50, 50 + i * 50, 200, 25, m->GetName(), NH3Dlg::Text::BIG, 1, mod.GetId(),
