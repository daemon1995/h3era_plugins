...
constexpr int DLG_HEIGHT_ADD = 86;
...
_LHF_(Battle_Dlg_Create)
{
    ...
    if (dlg_panel && dlg_panel->GetHeight() > 200)
    {
        ...
        // add new modified BG
        H3DlgPcx *_bg = H3DlgPcx::Create(0, 70 + DLG_HEIGHT_ADD, creature_stat_bg[0]->width,
                                         creature_stat_bg[0]->height, 777, nullptr);
        static int call_counter = 0;
        bool side = call_counter++ & 1;
        int player_id =
            P_CombatManager->hero[side] > 0 ? P_CombatManager->heroOwner[side] : P_CombatManager->heroOwner[!side];
        ...
        for (int i = 4; i < 6; i++)
        {
            ...
            temp.Append(i);
            text = H3DlgText::Create(x_pos, y_pos, 60, 12, Era::tr(temp.String()), NH3Dlg::Text::TINY, 1, item_id++,
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            H3String temp = "gem_plugin.combat_dlg.";
            temp.Append(i);
            text = H3DlgText::Create(x_pos, y_pos, 30, 12, Era::tr(temp.String()), NH3Dlg::Text::TINY, 1, item_id++,
                                     eTextAlignment::MIDDLE_LEFT);
            dlg_panel->AddItem(text);

            text = H3DlgText::Create(x_pos, y_pos, 60, 12, "", NH3Dlg::Text::TINY, 1, item_id++,
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            text =
                H3DlgText::Create(9, y_pos, 60, 12, "", NH3Dlg::Text::TINY, 1, item_id++, eTextAlignment::MIDDLE_RIGHT);
            dlg_panel->AddItem(text);
        }
        ...
        for (int i = 0; i < 12; i++)
        {
            ...
            text = H3DlgText::Create(x_pos + 10, y_pos + 8, 24, 12, "", NH3Dlg::Text::TINY, 1, item_id++,
                                     eTextAlignment::MIDDLE_RIGHT);
            dlg_panel->AddItem(text);
        }
    }
...
void Hel_SetActionType(H3CombatCreature *stack, H3DlgText *text)
{
...
_LHF_(MonPreview::H3CombatMonsterPanel_Prepare)
{

    c->return_address = 0x46DA0E;

    return SKIP_DEFAULT;

    if (IntAt(0x698818) == 1 && 0)
    {
        // stack_dlg_shown = true;

        H3CombatMonsterPanel *dlg_panel = reinterpret_cast<H3CombatMonsterPanel *>(c->edi);
        if (dlg_panel)
        {
            ...
            for (H3DlgItem *it : dlg_panel->GetItems())
            {
                ...
                {
                case 2207:
                    hero = dlg_panel == P_CombatManager->dlg->leftMonsterPopup ? P_CombatManager->hero[0]
                                                                               : P_CombatManager->hero[1];

                    if (stack->type == eCreature::BALLISTA && hero)
                    {
                        UINT8 unsignAttack = static_cast<UINT8>(hero->primarySkill[0]);
                        str.Append((unsignAttack + 1) * stack->info.damageLow);
                        if (stack->info.damageLow != stack->info.damageHigh)
                            str.Append("-").Append((unsignAttack + 1) * stack->info.damageHigh);
                ...
                case 2212:
                    ...
                    // str = std::to_string(losses).c_str();
                    it->Cast<H3DlgText>()->SetText(std::to_string(losses).c_str());

                    break;
                case 4005:
                    // def = it->Cast<H3DlgDef>();
                    text = it->Cast<H3DlgText>();
                    Hel_SetActionType(stack, text);
                    break;
                case 4007:
                    text = it->Cast<H3DlgText>();
...
void MonPreview::CreateResizedSpellEffectPictures()
{
    ...
    int width = spellsDef->widthDEF;
    int height = spellsDef->heightDEF;

    constexpr int scale = 2; // Scale of the pic

    int _width = width / scale;
    int _height = height / scale;

    const int framesNum = spellsDef->groups[0]->count;
    resizedSpellPictures.Resize(framesNum);

    if (H3LoadedPcx16 *buf = H3LoadedPcx16::Create(width, height))
    {
        for (size_t i = 0; i < framesNum; i++)
        {
            spellsDef->DrawToPcx16(0, 1, buf, 0, 0);

            H3LoadedPcx16 *resized = H3LoadedPcx16::Create(_width, _height);

            resized::H3LoadedPcx16Resized::DrawPcx16ResizedBicubic(resized, buf, width, height, 0, 0, _width, _height);
            resizedSpellPictures[i] = resized;
        }

        buf->Destroy();
    }
}
void MonPreview::CreatePanelBackGrounds()
{
    H3PcxLoader realbg("CCrPop.pcx");
    constexpr int heightOffset = 70;

    const int width = realbg->width;
    const int height = realbg->height - heightOffset;
    constexpr int newWidth = 298;
    for (size_t i = 0; i < 2; i++)
    {
        creaturePanelBackgrounds[i] = H3LoadedPcx::Create(h3_NullString, width, height);
        realbg->DrawToPcx(0, heightOffset, width, height, creaturePanelBackgrounds[i], 0, 0, 1);
    }
}
...
void CreateResources()
{
    ...
    H3DefLoader spell_int_def("spellint.def");
    int width = spell_int_def->widthDEF;
    int height = spell_int_def->heightDEF;

    // CreateSmallerPics(spell_int_def.Get(), smaller_spellint, spell_int_def->groups[0]->count,
    // spell_int_def->widthDEF, spell_int_def->heightDEF);
...
H3CombatMonsterPanel *__stdcall MonPreview::H3CombatMonsterPanel_Ctor(HiHook *h, H3CombatMonsterPanel *panel, int x,
                                                                      ...
                                                                      DWORD type)
{
    ...
    if (type == 1)
    {
        ...
        height += DLG_HEIGHT_ADD;
        y -= DLG_HEIGHT_ADD;
        y += 13;
    }

    auto result = THISCALL_7(H3CombatMonsterPanel *, h->GetDefaultFunc(), panel, x, y, width, height, parent, type);

    if (type == 1)
    {
        // auto& backs = Get().creaturePanelBackgrounds;
        H3PcxLoader realbg("CCrPop.pcx"); // : ("");
        constexpr int heightOffset = 70;
        ...
        //	for (size_t i = 0; i < 2; i++)
        {
            auto *back = H3LoadedPcx::Create(h3_NullString, width, height);
            realbg->DrawToPcx(0, heightOffset, width, height, back, 0, 0, 1);
            H3DlgPcx *_bg = H3DlgPcx::Create(0, 70 + DLG_HEIGHT_ADD, width, height, 777, nullptr);
            static bool call_counter = 0;
            bool side = call_counter++ & 1;
            int player_id =
                P_CombatManager->hero[side] > 0 ? P_CombatManager->heroOwner[side] : P_CombatManager->heroOwner[!side];
            _bg->SetPcx(back);
    ...
    {
        for (auto &it : panel->GetItems())
        {
            if (auto def = it->Cast<H3DlgDef>())
            {
                // def->SetFrame(12);
            }
            // it->SetX(500);
...
void MonPreview::CreatePatches()
{
    if (!m_isInited)
    {
        ...
        _pi->WriteLoHook(0x46D97A, H3CombatMonsterPanel_Prepare);
