...
void CurrentDlg_CreateDlgCallPcx(H3BaseDlg* dlg, int x, int y, const DlgStyle* style)
{
	...
	auto* captionPcx = H3DlgTextPcx::Create(x, y, style->WIDGET_WIDTH, style->WIDGET_HEIGHT, LocaleHandler::GetDisplayedName(), NH3Dlg::Text::MEDIUM, nullptr, 7, LOCALEDLG_BUTTON_ID);
...
void LanguageSelectionDlg::CreateDlgItems()
{
	...
	for (size_t i = 0; i < m_LOCALES_TO_DRAW; i++)
	{
		...
		auto textPcx = H3DlgTextPcxLocale::Create(0, y, WIDGET_WIDTH, WIDGET_HEIGHT, m_localeHandler->LocaleAt(i), NH3Dlg::Text::MEDIUM, m_style->localeBackgroundLoadedPcx, 1, i + FIRST_SELECTION_WIDGET_ID);
		AddItem(textPcx);
	}
