...
{
    return CDECL_3(char *, 0x77710B, StrNum, ColNum, 0x2860724);
}
...
void SetNewSecSkillsFrames(H3BaseDlg *dlg, int increment = 0)
{
    ...
    for (int i = 0; i < 6; i++)
    {
        if (H3DlgDef *SSkillDef = dlg->GetDef(60 + i))
        {
            ...
            if (increment)
            {
                auto res = std::find_if(std::begin(npcSSVec), std::end(npcSSVec),
                                        [currentFrameId](int a) { return a == currentFrameId; });
            ...
            o_dlgNPC->SpecBonusHints[i] = newFrameId % 2
                                              ? Get_ITxt(28 + strId, 1)
                                              : Get_ITxt(28 + strId, 2); // new hint depends on learned or not
...
int GetNpcSSkills(WoG::NPC *npc)
{
    ...
    for (int i = 0; i < NPC_MAX_SKILLS; i++)
    {
        ...
        else if (!(npcBannedSecSkillsBits & mask) &&
                 NPC_CalcSkillMayBe(npc, i)) // if skill isn't banned and can be achieved
...
int __stdcall Before_WndNPC_DLG(LoHook *h, HookContext *c) // before dlg run
{
    ...
    {
        if (auto sSkillDef = dlg->GetDef(60))
        {

            const int yPos = sSkillDef->GetY() + 3; // get current any SS frame y pos

            const int xLeftPos = sSkillDef->GetX() - 25; // get current first SS frame x pos
            H3DlgCustomButton *leftBttn =
                H3DlgCustomButton::Create(xLeftPos, yPos, NPC_DLG_LEFT_BTTN_ID, "hsbtns3.def", WogNPC_BTTNS_Proc, 0, 1);
            leftBttn->AddHotkey(eVKey::H3VK_LEFT); // adding 2 HK for lazy daemon
            leftBttn->AddHotkey(eVKey::H3VK_A);

            if (auto rightBttnDef = dlg->GetDef(65))
            {

                int xRighttPos = rightBttnDef->GetX() + 55; // get current last SS frame x pos

                H3DlgCustomButton *rightBttn = H3DlgCustomButton::Create(xRighttPos, yPos, NPC_DLG_RIGHT_BTTN_ID,
                                                                         "hsbtns5.def", WogNPC_BTTNS_Proc, 0, 1);
...
_LHF_(HooksInit)
{
    ...
    DWORD pluginHookAddress = wndPlugin.NeedleSearch<7>({0x02, 0x00, 0x00, 0x7C, 0x83, 0x8B, 0x7D}, 15);
