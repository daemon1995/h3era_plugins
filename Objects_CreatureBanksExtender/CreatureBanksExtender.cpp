...
_LHF_(CreatureBanksExtender::AIHero_GetCreatureBankItemWeight)
{
    ...
    {
        float fExperience = static_cast<float>(experience) * hero->GetLearningPower();
...
_LHF_(CreatureBanksExtender::CrBank_AfterCombatWon)
{
    ...
    currentCreatureBank.experiencePointsToAdd +=
        static_cast<int>(static_cast<float>(customBank->experience) * hero->GetLearningPower());
                                                                                 ...
                                                                                 const char *rewardText) noexcept
{
    ...
    {
        finalMessage = H3String::Format("{%s}", extender::GetObjectName(mapItem));
    }
...
BOOL CreatureBanksExtender::RMGDlg_ShowCustomObjectHint(const H3ObjectAttributes &info, H3String &defaultText) noexcept
{
