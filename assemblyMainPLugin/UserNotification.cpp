...
void __stdcall UserNotification::AdvMgrDlg_RedrawLog(HiHook *h, H3AdventureMgrDlg *dlg, bool redrawLog)
{
    ...
    if (updateInfo->m_needToCheck)
    {
        ...
        if (timeDifference >= 5000 && AssemblyInformation::Get().CompareVersions())
        {
...
char __stdcall UserNotification::AdvMgr_DrawMap(HiHook *h, H3AdventureManager *advMan, const __int16 mapX,
                                                const __int16 mapY, const __int16 mapZ, const int hotSeat,
                                                char updateInfoPanel)
{
...
void __stdcall OnDrawActionPlayCastSpell(HiHook *h, H3CombatManager *mgr, eSpell spellId, int pos, int casterKind,
                                         int pos2, int skilLevel, int spellPower)
{
