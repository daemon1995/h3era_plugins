...
CreatureBanksManager::CreatureBanksManager()
	:ObjectsExtender(globalPatcher->CreateInstance("EraPlugin.CreatureBanksManager.daemon_n"))
{
...
CreatureBanksManager& CreatureBanksManager::Get()
{
	static CreatureBanksManager _instance;
	return _instance;
}
...
void CreatureBanksManager::CreatePatches()
{
	//m_isInited = true;
...
void __stdcall CreatureBanksManager::LoopSoundManager::OnGameLeave(Era::TEvent* event)
{
...
int __stdcall CreatureBanksManager::LoopSoundManager::AdvMgr_MapItem_Select_Sound(HiHook* h, H3AdventureManager* adv, const int x, const int y, const int z)
{
...
_LHF_(CreatureBanksManager::CrBank_BeforeCombatStart)
{
...
signed int __stdcall  CreatureBanksManager::CrBank_CombatStart(HiHook* h, UINT AdvMan, UINT PosMixed, UINT attHero, UINT attArmy, int PlayerIndex,
...
const int CreatureBanksManager::GetCreatureBankId(const int objType, const int objSubtype) noexcept
{
	int cbId = -1;
	switch (objType)
	{
...
const int CreatureBanksManager::GetCreatureBankObjectType(const int cbId) noexcept
{
...
{
	if (id < CreatureBanksManager::Get().creatureBanks.Size())
	{
		sprintf(buff, "%s", CreatureBanksManager::Get().creatureBanks.setups[id].name.String());
	}
...
void CreatureBanksManager::SetRmgObjectGenData(const int objectSubtype)  noexcept
{
	return;
	const int value = EraJS::readInt(H3String::Format("RMG.objectGeneration.16.%d.value", objectSubtype).String());
	const int density = EraJS::readInt(H3String::Format("RMG.objectGeneration.16.%d.density", objectSubtype).String());
	if (value || density)
	{
		RMGObjectInfo rmgObjectInfo(eObject::CREATURE_BANK, objectSubtype);
...
void CreatureBanksManager::AferLoadingObjectTxtProc(const INT16* maxSubtypes)
{
	...
	{		// set new Creature Bank Setups data at native array address
		...
		_pi->WriteDword(0x67029C, int(Get().creatureBanks.setups.data()));
	}
}
...
const int CreatureBanksManager::GetBanksSetupFromJson(const INT16 maxSubtype)
{
...
const UINT CreatureBanksManager::CreatureBank::Size() const noexcept
{
	return m_size;
}

void CreatureBanksManager::CreatureBank::CopyDefaultData(const size_t defaultSize)
{
...
void CreatureBanksManager::Resize(UINT16 m_size) noexcept
{
...
void CreatureBanksManager::Reserve(UINT16 m_size) noexcept
{
...
void CreatureBanksManager::ShrinkToFit() noexcept
{
