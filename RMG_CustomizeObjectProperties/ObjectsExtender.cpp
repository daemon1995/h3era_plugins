...
namespace extender
{
	...
	void __stdcall ObjectsExtender::H3GameMainSetup__LoadObjects(HiHook* h, const H3MainSetup* setup)
	{
		...
		{	// iterate all the objects types entries
			...
			for (size_t objSubtype = 0; objSubtype < maxSubtype; objSubtype++)
			{
				...
				do
				{
					//				std::string str = EraJS::read(H3String::Format("RMG.objectGeneration.%d.%d.properties.%d", objType, objSubtype, propertyIdCounter++).String(), readSuccess);
					LPCSTR str = EraJS::read(H3String::Format("RMG.objectGeneration.%d.%d.properties.%d", objType, objSubtype, propertyIdCounter++).String(), readSuccess);
					if (readSuccess)
					{
	...
	_LHF_(ObjectsExtender::H3AdventureManager__ObjectVisit_SoundPlay)// (HiHook* h, const int objType, const int objSetup)
	{
		...
		if (auto mapItem = reinterpret_cast<H3MapItem*>(c->ebx))
		{
			...
			H3String soundFileName = EraJS::read(H3String::Format("RMG.objectGeneration.%d.%d.sound.enter", mapItem->objectType, mapItem->objectSubtype).String(), readSuccess);
			//if there is entry
			if (readSuccess)
			{
