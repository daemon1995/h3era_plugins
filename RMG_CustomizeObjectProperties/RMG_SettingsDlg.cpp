...
namespace rmgdlg
{
	...
	const std::vector<std::vector<std::pair<H3ObjectAttributes, H3LoadedPcx16*>>*> RMG_SettingsDlg::m_objectAttributes = { &m_banks, &m_commonObjects, &m_dwellings, &m_wogObjects };
	...
	const std::vector<std::vector<std::pair<H3ObjectAttributes, H3LoadedPcx16*>>*>& RMG_SettingsDlg::GetObjectAttributes() noexcept
	{
		...
		:H3Dlg(width, height, x, y, false, false), m_settings(nullptr), m_currentPage(nullptr)
	{
		...
		for (size_t i = 0; i < 5; ++i)
		{
			...
			auto* bttn = H3DlgDefButton::Create(_x, _y, NItemIDs::BUTTON_FIRST + i, data.defName, 0, 1, data.closeDlg, data.hotkeys[0]);
			...
			if (bttn)
			{
				...
				// set hints
				bttn->SetHints(
					EraJS::read(H3String::Format("RMG.text.dlg.buttons.%s.hint", data.jsonName).String()),
					EraJS::read(H3String::Format("RMG.text.dlg.buttons.%s.rmc", data.jsonName).String()),
					false);
		...
		for (size_t i = 0; i < pagesNum; i++)
		{
			...
			H3DlgCaptionButton* captionbttn = H3DlgCaptionButton::Create(_x, _y, NItemIDs::PAGE_FIRST + i, "RMGmenbt.def", EraJS::read(h3_TextBuffer), NH3Dlg::Text::MEDIUM, 0, 0, 0, eVKey::H3VK_1 + i, eTextColor::HIGHLIGHT);

			captionbttn->SetHints(
				EraJS::read(H3String::Format("RMG.text.dlg.pages.%d.hint", i).String()),
				EraJS::read(H3String::Format("RMG.text.dlg.pages.%d.rmc", i).String()),
				false);
		...
		if (bttns[1] && bttns[2])
		{
			...
			hintBar = reinterpret_cast<H3DlgHintBar*>(H3DlgTextPcx::Create(_x, _y, _w, _h, h3_NullString, NH3Dlg::Text::SMALL, "rmghntbr.pcx", eTextColor::REGULAR));
			AddItem(hintBar);
		}
		...
		for (auto& page : m_pages)
		{
			...
			{// try to cast to map object page
				...
				for (RMGObject& object : mapObjectPage->RMGObjects)
				{
					...
					if (saveIni)
					{
						H3String sectionName = H3String::Format("%d_%d_%d", info.type, info.subtype, zoneType);// , obj.attributes->defName.String());
	...
	void RMG_SettingsDlg::ObjectsPage::CreateVerticalScrollBar()
	{
		...
		if (PANELS_NUM < DATA_NUM)
		{
			...
			verticalScrollBar = H3DlgScrollbar::Create(dlg->widthDlg - 24, scrollBarY, 16, lastPanel->backgroundPcx->GetY() + lastPanel->backgroundPcx->GetHeight() - scrollBarY, itemId, DATA_NUM + 1 - PANELS_NUM, VerticalScrollBarProc, true, 1, true);
	...
	void RMG_SettingsDlg::ObjectsPage::CreateHorizontalScrollBar()
	{
		...
		if (dlgPanels.size())
		{
			...
			if (editsNum > 4)
			{
				...
				horizontalScrollBar = H3DlgScrollbar::Create(x, y, lastPanel->backgroundPcx->GetWidth() - x + padding, 16, 101, 5, HorizontalScrollBarProc, true, 1, true);
	...
	RMG_SettingsDlg::BanksPage::BanksPage(H3DlgCaptionButton* captionbttn, const std::vector<std::pair<H3ObjectAttributes, H3LoadedPcx16*>>& data, const BOOL ignoreSubtypes)
		...
		:x(x), y(y), visible(false), parentPage(parent)
	{
		...
		pictureItem = H3DlgPcx16::Create(DLG_OBJ_PCX_X_OFFSET + 10, y + 3, PCX_WIDTH, PCX_HEIGHT, itemId++, 0); // create dlg pcx w/o picture
		items.emplace_back(pictureItem);

		const int DLG_OBJ_NAME_X_OFFSET = PCX_WIDTH + pictureItem->GetX();

		constexpr int textPadding = 20;
		objectNameItem = H3DlgText::Create(DLG_OBJ_NAME_X_OFFSET + 7, y, (FIELD_WIDTH - PCX_WIDTH) / 3 - textPadding, FIELD_HEIGHT, h3_NullString, h3::NH3Dlg::Text::MEDIUM, 1, itemId++);
		...
		{
			if (edits[i] = H3DlgEdit::Create(DLG_EDITS_X_OFFSET + 100 * i + 22, y + 15, 52, 22, MAX_TEXT_LENGTH, h3_NullString, h3::NH3Dlg::Text::MEDIUM, 1, eTextAlignment::MIDDLE_CENTER, "RMG_edit.pcx", itemId++, 4, 1, 1))
	...
	RMG_SettingsDlg::ObjectsPage::ObjectsPage(H3DlgCaptionButton* captionbttn, const std::vector<std::pair<H3ObjectAttributes, H3LoadedPcx16*>>& attributes, const BOOL ignoreSubtypes)
	...
	BOOL RMG_SettingsDlg::ObjectsPage::ShowObjectExtendedInfo(const ObjectsPanel* panel, const H3Msg& msg)  const noexcept
	{
		...
		if (clickedItem == panel->pictureItem
			|| clickedItem == panel->objectNameItem)
		{
				...
				if (cbSetup.states[i].chance > 0)
				{
					...
					for (size_t j = 0; j < 7; j++)
					{
						...
						if (montype != eCreature::UNDEFINED)
						{
							...
							ddl.CreateText(_x, _y + def->GetHeight() + 2, def->GetWidth(), 20, Era::IntToStr(monNum).c_str(), NH3Dlg::Text::SMALL, eTextColor::WHITE, -1);
	...
	BOOL RMG_SettingsDlg::ObjectsPage::Proc(H3Msg& msg)
	{
			...
			)
		{
			...
			const bool itemIsInHeader = std::find(pageHeader->items.begin(), pageHeader->items.end(), clickedItem) != pageHeader->items.end();
					...
					if (dlgPanel->visible)
					{
							...
							&& dlgPanel->items.end() != std::find(dlgPanel->items.begin(), dlgPanel->items.end(), clickedItem))
						{
							if (msg.subtype == eMsgSubtype::LBUTTON_DOWN
								&& (dlgPanel->enabledCheckBox == clickedItem
									|| dlgPanel->pictureItem == clickedItem
							...
							for (size_t i = 0; i < SIZE; ++i)
							{
								//	dlgPanel->rmgObject->objectInfo.data[i + 1] = dlgPanel->edits[i]->GetH3String().ToSigned();
							}
	...
	//H3Dlgsc
	RMG_SettingsDlg::Page::Page(H3DlgCaptionButton* captionbttn)
		:captionbttn(captionbttn), verticalScrollBar(nullptr)
	{
		//resizedPcx16.RemoveAll();
		id = dlg->m_pages.size();
		...
		:attributes(&attributes), objectPcx(objectPcx)
	{
		...
		if (H3LoadedPcx16* tempPcx = H3LoadedPcx16::Create(srcWidth, srcHeight))
		{
			...
			frame->DrawToPcx16(frame->marginLeft, frame->marginTop, frame->frameWidth, frame->height, tempPcx, 0, 0, def->palette565);
			...
			//			resized::H3LoadedPcx16Resized::DrawPcx16ResizedBicubic(resizedPcx, tempPcx, srcWidth, srcHeight, dstX, dstY, dstWidth, dstHeight);
	...
	RMG_SettingsDlg::ObjectsPage::PageHeader::PageHeader(const int x, const int y, const int width, const int height, const int objectsNum)
	{
		items.clear();
		items.reserve(15);
		...
		for (size_t i = 0; i < 6; i++)
		{
			...
			H3DlgText* text = H3DlgText::Create(_x[i] + x, y + 2, _w[i], height - 4, EraJS::read(H3String::Format("RMG.text.dlg.header.%d.name", i).String()), NH3Dlg::Text::SMALL, 1, rmgdlg::NItemIDs::HEADER_FIRST + i);
			if (i == 1)
			{
			...
			text->SetHints(
				EraJS::read(H3String::Format("RMG.text.dlg.header.%d.hint", i).String()),
				EraJS::read(H3String::Format("RMG.text.dlg.header.%d.rmc", i).String()),
				false);
			auto* frame = H3DlgFrame::Create(text, color, 0, 1);
			items.emplace_back(frame);
			items.emplace_back(text);

			H3DlgDef* arrow = H3DlgDef::Create(text->GetX() + text->GetWidth() - _arrowOffset[i], y + 4 + ((height - 16) >> 1), text->GetID(), defName, 2);
	...
	RMG_SettingsDlg::MiscPage::MiscPage(H3DlgCaptionButton* captionbttn, const std::vector<std::pair<H3ObjectAttributes, H3LoadedPcx16*>>& data)
	...
	RMG_SettingsDlg::DwellingsPage::DwellingsPage(H3DlgCaptionButton* captionbttn, const std::vector< std::pair<H3ObjectAttributes, H3LoadedPcx16*>>& data)
		:ObjectsPage(captionbttn, data)
	{
	...
	void GetObjectPrototypesByTypes(std::vector<std::pair<H3ObjectAttributes, H3LoadedPcx16*>>& dataVec, const int* arr, const size_t size)
	{
		dataVec.clear();
		...
		for (size_t i = 0; i < size; i++)
		{
			...
			{
				const auto result = std::find_if(dataVec.cbegin(), dataVec.cend(), [&](const std::pair<H3ObjectAttributes, H3LoadedPcx16*>& atr) ->bool
					{
						return atr.first.type == obj.type && atr.first.subtype == obj.subtype;// (ignoreSubtypes ? obj.subtype != 0 : atr.subtype == obj.subtype);
					});
	...
	void FindDataInObjectGeneratorsList(std::vector<std::pair<H3ObjectAttributes, H3LoadedPcx16*>>& dataVec)
	{
		...
		if (attributesList.size())
		{
			...
			{
				auto result = std::find_if(attributesList.begin(), attributesList.end(), [&](const std::pair<H3ObjectAttributes, H3LoadedPcx16*>& atr) ->bool
					{
						return atr.first.type == rmgGenObj->type && atr.first.subtype == rmgGenObj->subtype;// (ignoreSubtypes ? obj.subtype != 0 : atr.subtype == obj.subtype);
					});
	...
	*/
	...
	void __stdcall RMG_SettingsDlg::NewScenarioDlg_Create(HiHook* hook, H3SelectScenarioDialog* dlg, const DWORD dlgCallType)
	{
		...
		if (dlgCallType == 0) // 0 new/ 1 load/2 from map
		{
			...
			if (H3DlgCaptionButton* bttn = dlg->CreateCaptionButton(246, 291, 128, 20, SELECTION_DLG_RMG_SETTINGS_BUTTON_ID, *reinterpret_cast<char**>(0x57A93B + 1), EraJS::read("RMG.text.buttons.setup.name"), h3::NH3Dlg::Text::SMALL, 0, 0, 0, 0, eTextColor::HIGHLIGHT)) {
			...
			if (H3DlgCustomButton* randomGameButton = H3DlgCustomButton::Create(dlg->GetWidth() - 120, dlg->GetHeight() - 88, "RanRand.def", SelectScenarioDlgRandomizeProc, 0, 1))
			{
