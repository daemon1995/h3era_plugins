...
namespace warehouses
{
		...
		if (H3MapItem* mapItem = reinterpret_cast<H3MapItem*>(c->esi))
		{
			...
			if (auto data = H3MapItemWarehouse::GetWarehouse(mapItem))
			{
				...
				if (!isVistedByPlayer || data->resourceAmount)
				{
					...
					const int aiResWeight = static_cast<int>(player->resourceImportance[resType] * resNum);// static_cast<double*>()
					if (aiResWeight)
					{
						c->eax = aiResWeight;
	...
	void ShowMessage(const H3MapItem* mapItem, const int resType, const int resNum)
	{
		...
		if (skipMapMessage)
		{
			if (resNum < 1)
			{
				objName.Append(EraJS::read(H3String::Format("RMG.objectGeneration.%d.text.visited", mapItem->objectType).String()));
			}
		...
		else
		{
			objName.Append(
				EraJS::read(resNum > 0 ?
					H3String::Format("RMG.objectGeneration.%d.text.visit", mapItem->objectType).String()
	...
	_LHF_(WarehousesExtender::H3AdventureManager__ObjectVisit)
	{
		if (auto mapItem = reinterpret_cast<H3MapItem*>(c->edi))
		{
			if (auto data = H3MapItemWarehouse::GetWarehouse(mapItem))
			{
				...
				const INT16 resNum = data->resourceAmount ? resType == eResource::GOLD ? data->resourceAmount * 500 : data->resourceAmount : -1;
