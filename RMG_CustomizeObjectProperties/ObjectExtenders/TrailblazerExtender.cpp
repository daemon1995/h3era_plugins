...
_LHF_(js_SetTrailblazerBonus)
{
    if (const H3Hero* hero = P_ActivePlayer->GetActiveHero())
    {
        const int terrainType = c->edi;
        if (terrainType == TRAILBLAZER_TERRAIN_TYPE)
        {
            const bool isVisitedByHero = H3MapItemTrailblazer::IsVisitedByHero(hero);
            if (isVisitedByHero)
            {
...
const int js_HotaCalcStepCost(const int stepCost, const int pathfindSkill, char direction)
{
...
{
    const bool hotaPathfindingOpt = Era::GetAssocVarIntValue(HOTA_PATHFINDING_OPT);
    ...
    if (H3Hero* hero = P_ActivePlayer->GetActiveHero())
    {
        ...
        if (hero->WearsArtifact(ART_WAYFARERS_BOOTS))
        {
            wfBootsEquipped = true;

            if (!hotaPathfindingOpt)
            {
