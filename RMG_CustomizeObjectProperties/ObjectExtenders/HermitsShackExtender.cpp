...
//int NeedExpoToNextLevel(int lvl)
//{
//	return FASTCALL_1(int, 0x04DA690, lvl);
//}

BOOL HermitsShackExtender::SetAiMapItemWeight(H3MapItem *mapItem, const H3Hero *hero, const H3Player *player,
                                              int &aiMapItemWeight) const noexcept
{
     ...
     if (auto hermitsShack = H3MapItemHermitsShack::GetFromMapItem(mapItem))
    {
        ...
        if (!isVisitedByHero)
        {
            ...
            aiMapItemWeight = static_cast<float>(h3functions::NeedExpoToNextLevel(hero->level)) * hero->AI_experienceEffectiveness;
