...
int PrepareAndShowArtifactsDlg(H3Hero *hero, int slot, H3DlgItem *it, std::vector<H3Artifact> &heroArts, int scrollsNum)
{
    ...
    int itemsInRow =
        std::max(scrollsNum, artsNoScrollsNum) > 7 ? 8 : std::max(scrollsNum, artsNoScrollsNum); // set max items in row
    ...
    artToChooseDlg->FrameRegion(0, 0, width, height, false,
                                hero->owner); // set player color frame (for PvP trade mostly)
        ...
        if (it) // if item
        {
            ...
            if (vSlot >= eArtifactSlots::HEAD && vSlot <= eArtifactSlots::MISC5) // if slot is in range
            {
                if (msg->subtype == eMsgSubtype::LBUTTON_DOWN && msg->flags == eMsgFlag::SHIFT) // only for shift + LMC
                {
                    ...
                    msg->subtype = eMsgSubtype::MOUSE_WHEEL_BUTTON_UP; // disable next shift + LMC reaction to prevent
                                                                       // other reactions
                }
                // play sound
                ...
                if (heroArts.size())
                {
                    ...
                    if (PrepareAndShowArtifactsDlg(hero, vSlot, it, heroArts, scrollsCounter) !=
                        eArtifact::NONE) // if chosen any art
                        return ART_PLACED;
                }
            }
        }
