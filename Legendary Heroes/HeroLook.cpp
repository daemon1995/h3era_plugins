...
void HeroLook::ResetSettings()
{
	...
	for (int i = 0; i < HEROES_MAX_AMOUNT; i++)
	{
		...
		UINT16* values[SIZE] = { &heroLook[i].faction ,&heroLook[i].portraitIndex ,&heroLook[i].background };// , & heroLook[i].original};
...
DllExport void SetHeroLook(const UINT heroId, const int faction, const int portraitIndex, const int background, const int ornament, const bool original, bool save = 0)
{
...
DllExport void GetHeroLook(const UINT heroId, int *faction, int *portraitIndex, int *background, int *ornament, bool* original)
{

	if (heroId < HEROES_MAX_AMOUNT)
	{
		auto &h = HeroLook::heroLook[heroId];
		if (faction) *faction = h.faction;
		if (portraitIndex) *portraitIndex = h.portraitIndex;
		if (background) *background = h.background;
		if (ornament) *ornament = h.ornament;
...
void HeroLook::LoadIniSettings(HeroLook* heroes)
{
	...
	for (size_t i = 0; i < HEROES_MAX_AMOUNT; i++)
	{
		...
		//	P_HeroInfo[i].smallPortrait = "nhs1_1.pcx";// H3String::Format("nhl%d_%d.pcx", *values[0], *values[1]).String();

		//	SetPortraitName(i, H3String::Format("nhl%d_%d.pcx", *values[0], *values[1]).String(), H3String::Format("nhl%d_%d.pcx", *values[0], *values[1]).String());
		}
