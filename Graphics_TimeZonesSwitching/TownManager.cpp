...
namespace timezone
{
	...
	void __stdcall TownManager::TownMgrDlg_BuildingsWindowsRedraw(HiHook* h, H3WindowManager* wndMgr, const int x, const int y, const int w, const int height)
	{
		...
		for (auto& data : defDrawData)
		{
			if (!data.defButton->IsVisible())
			{
				...
				if (readSucces && !defName.Empty())
				{
					...
					*reinterpret_cast<H3LoadedDef**>(reinterpret_cast<char*>(data.defButton) + 0x30) = H3LoadedDef::Load(defName.String());
